...
flightSuretyApp.events.OracleRequest({
    ...
    oraclesIndexList.forEach((indexes) => {
      let oracle = oracleAccountsArr[idx];
  ...
  if(defaultStatus == STATUS_CODE_LATE_AIRLINE){
    flightSuretyData.methods.creditInsurees(flight).call({ from: oracle}, (error, result) => {
      if(error){
        ...
        console.log("Credit amount set for insurees.");
        // console.log(payload);
        // console.log(result);
      }
    });
...
function getOracleAccounts() {
  return new Promise((resolve, reject) => {
    web3.eth.getAccounts().then(accountList => {
      ...
      oracleAccountsArr = accountList.slice(20, 20+TEST_ORACLES_COUNT);
    }).catch(err => {
      reject(err);
    }).then(() => {
      resolve(oracleAccountsArr);
    });
  });
}

function initialiseOracles(accounts) {
  return new Promise((resolve, reject) => {
