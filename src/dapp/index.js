...
(async() => {
    ...
    let contract = new Contract('localhost', () => {
        ...
        DOM.elid('default-air').value = contract.owner;
        DOM.elid('selected-airline-name').value = 'Default Airlines';
        DOM.elid('selected-airline-address').value = contract.owner;

        // Read transaction
        contract.isOperational((error, result) => {
            display('Logs', 'Is Contract Operational ?', [ { label: 'Operational Status', error: error, value: result} ]);
        });
            ...
            contract.pay((error, result) => {
                if(error){
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'error',
                        title: 'Error! Could not withdraw the credit.',
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: `Successfully withdrawed ${creditDisplay.value} wei!`,
                        showConfirmButton: false,
                        timer: 3000
                      })
    ...
    DOM.elid('statusButton').addEventListener('click', async(e) => {
        ...
        let myJson = ''
        await axios.get(`http://localhost:3000/api/status/${buttonValue}`).then(res => {
            myJson = res
        })
        console.log(myJson)
        display('', 'Default flights status change submited to server.', [ { label: 'Server response: ', value: myJson.data.message} ]);
    })
...
function flightDisplay(flight, destination, airlineName, time) {
    ...
    var emptyRow = DOM.elid("empty-row")
    if(flightCount == 0){
        emptyRow.remove();
    }
    flightCount++;
...
function getTimeFromTimestamp(timestamp) {
    return new Date(timestamp * 1000).toLocaleTimeString("en-IN").slice(0, -3);
}